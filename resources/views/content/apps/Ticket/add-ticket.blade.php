...
document.addEventListener("DOMContentLoaded", function () {
    ...
    $('#pelangganSelect').select2({
        placeholder: 'Cari nama, ID, atau No WA pelanggan...',
        allowClear: true,
        width: '100%',
        ajax: {
    ...
    $('#pelangganSelect').on('select2:select', function (e) {
        ...
        if (!data) { resetCustomerForm(); return; }

        $('#pelanggan_id').val(data.id);
        $('#phone').val(data.nowhatsapp || '').addClass('autofill-highlight');
        $('#nama_pelanggan').val(data.nama || '').addClass('autofill-highlight');
        $('#paket_pelanggan').val(data.paket || '').addClass('autofill-highlight');

        // Alamat
        const parts = [];
        const jalan = data.alamat_jalan || '';
        const rt = data.rt || '';
        const rw = data.rw || '';
    ...
    $('#pelangganSelect').on('select2:clear', function (e) {
        resetCustomerForm();
    });
...
<div class="row g-4">
    ...
    <div class="col-12 col-lg-4">
        ...
        <div class="card mb-4">
            <div class="card-header-custom">
                <h5><i class="ri-tools-line"></i> Penugasan Karyawan</h5>
            </div>
            <div class="card-body p-4">
                <label for="user_id" class="form-label">
                    Pilih Karyawan <small class="text-muted fw-normal">opsional</small>
                </label>
                ...
                <div class="mt-3">
                    ...
                    <input type="text" class="form-control" id="assignment_date" name="assignment_date" value="{{ old('assignment_date') }}" placeholder="YYYY-MM-DD">
                </div>
