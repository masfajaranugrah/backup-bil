...
class AbsensiController extends Controller
{
    ...
    public function submit(Request $request)
    {
        ...
        $request->validate([
            ...
            'manual_time' => 'nullable|date_format:H:i',
        ]);

        // Jika user memilih jam manual (untuk checkout, lembur_in, lembur_out)
        $manualTimestamp = null;
        if ($request->filled('manual_time') && in_array($request->action, ['checkout', 'lembur_in', 'lembur_out'])) {
            $manualTimestamp = Carbon::parse($today . ' ' . $request->manual_time, 'Asia/Jakarta');
        }
        ...
        switch ($request->action) {
            case 'checkin':
                if ($attendance->time_in) {
                    return redirect()->route('karyawan.home')->with('error', 'Anda sudah melakukan check-in hari ini.');
                }
            ...
            case 'checkout':
                if (!$attendance->time_in) {
                    return redirect()->route('karyawan.home')->with('error', 'Belum melakukan check-in hari ini.');
                }
                if ($attendance->time_out) {
                    return redirect()->route('karyawan.home')->with('error', 'Anda sudah melakukan check-out hari ini.');
                }
                $attendance->time_out = $manualTimestamp ?? now()->setTimezone('Asia/Jakarta');
            ...
            case 'lembur_in':
                if ($attendance->lembur_in) {
                    return redirect()->route('karyawan.home')->with('error', 'Anda sudah mulai lembur hari ini.');
                }
                $attendance->lembur_in = $manualTimestamp ?? now()->setTimezone('Asia/Jakarta');
            ...
            case 'lembur_out':
                if (!$attendance->lembur_in) {
                    return redirect()->route('karyawan.home')->with('error', 'Belum mulai lembur hari ini.');
                }
                if ($attendance->lembur_out) {
                    return redirect()->route('karyawan.home')->with('error', 'Anda sudah selesai lembur hari ini.');
                }
                $attendance->lembur_out = $manualTimestamp ?? now()->setTimezone('Asia/Jakarta');
            ...
            default:
                return redirect()->route('karyawan.home')->with('error', 'Aksi tidak valid.');
        }
    }
    ...
    public function destroy(Absensi $absensi)
    {
        ...
        // Hanya administrator dan admin yang boleh menghapus data absensi
        if (!$user || !in_array($user->role ?? '', ['administrator', 'admin'])) {
            abort(403, 'Hanya administrator dan admin yang dapat menghapus data absensi.');
        }
