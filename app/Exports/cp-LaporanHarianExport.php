...
class LaporanHarianExport implements WithMultipleSheets
{
    ...
    public function sheets(): array
    {
        ...
        for ($day = 1; $day <= $daysInMonth; $day++) {
            ...
            // ── Data harian ──────────────────────────────────────────────
            $cashPayments   = $this->getCashPayments($date);
            $allExpenses    = $this->getExpenses($date);
            $kasReg         = $this->getKasRegistrasi($date);
            $bankPayments   = $this->getBankPayments($date);
     ...
     */
    private function getCashPayments(Carbon $date): Collection
    {
        // Gunakan startOfMonth() agar tanggal 31 Mei dikurangi 1 bulan tidak menjadi 1 Mei (overflow)
        $prevMonthDate = $date->copy()->startOfMonth()->subMonth();
     ...
     */
    private function getBankPayments(Carbon $date): Collection
    {
        // Gunakan startOfMonth() agar tanggal 31 Mei dikurangi 1 bulan tidak menjadi 1 Mei (overflow)
        $prevMonthDate = $date->copy()->startOfMonth()->subMonth();
