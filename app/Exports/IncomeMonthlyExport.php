...
class IncomeMonthlyExport implements FromView
{
    ...
    {
        $exportMonth = (int) ($this->month ?: now()->month);
        $exportYear = (int) ($this->year ?: now()->year);
        $previousPeriod = Carbon::createFromDate($exportYear, $exportMonth, 1)->subMonth();
        ...
        $incomes = Tagihan::where('tagihans.status_pembayaran', 'lunas')
            ...
            // Kecualikan periode tagihan bulan sebelumnya dari bulan export.
            ->where(function ($query) use ($previousPeriod) {
                $query->whereNull('tagihans.tanggal_mulai')
                    ->orWhereYear('tagihans.tanggal_mulai', '!=', $previousPeriod->year)
                    ->orWhereMonth('tagihans.tanggal_mulai', '!=', $previousPeriod->month);
            })
            ...
            ->select([
                ...
                DB::raw('COALESCE(rekenings.nama_bank, NULLIF(tagihans.type_pembayaran, ""), "Cash / Tunai") as type_pembayaran'),
            ])
        ...
        $bankTotals = Tagihan::where('tagihans.status_pembayaran', 'lunas')
            ...
            ->leftJoin('rekenings', 'rekenings.id', '=', 'tagihans.type_pembayaran')
            ->select([
                DB::raw('COALESCE(rekenings.nama_bank, NULLIF(tagihans.type_pembayaran, ""), "Cash / Tunai") as nama_bank'),
                DB::raw('SUM(COALESCE(tagihans.harga, pakets.harga, 0)) as total')
            ])
            ->groupBy('rekenings.nama_bank', 'tagihans.type_pembayaran')
            ->get();

        $monthLabel = Carbon::createFromDate($exportYear, $exportMonth, 1)->locale('id')->isoFormat('MMMM YYYY');
